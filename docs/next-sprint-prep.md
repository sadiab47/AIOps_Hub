...
1. **Session-to-Organization Binding**:
   - As we move to `v0.2.0`, consider whether a session should be bound to a _single_ organization context at a time, or if the access token should contain the user's _current active organization ID_ (allowing them to switch organizations by rotating their session/access token).
