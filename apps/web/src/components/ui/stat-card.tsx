...
export function StatCard({ label, value, icon: Icon, color = 'text-indigo-400', description }: StatCardProps) {
  return (
    <div className="p-6 rounded-2xl border border-white/[0.06] bg-white/[0.01] backdrop-blur-sm shadow-xl flex flex-col justify-between">
      ...
      {description && <div className="text-[10px] text-zinc-500 mt-3 font-medium">{description}</div>}
    </div>
  );
}
