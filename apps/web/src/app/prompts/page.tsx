...
export default function Prompts() {
  ...
  const handleAddSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert(
        err.response?.data?.error?.message || "Create prompt template failed",
      );
    } finally {
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          setEditorContent(
                            selectedPrompt.versions?.[idx]?.content || "",
                          );
                        }}
                    ...
                    <div className="space-y-1.5">
                      {variables.length === 0 ? (
                        ...
                        variables.map((v) => (
                          <div
                            key={v}
                            className="px-2.5 py-1.5 rounded-lg border border-indigo-500/10 bg-indigo-500/5 text-indigo-400 font-mono text-[10px] truncate"
                          >
                            {v}
                          </div>
                        ))
