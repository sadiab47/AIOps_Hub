...
export default function Playground() {
  ...
  const handleSend = (e: React.FormEvent) => {
    ...
    setMessages(prev => [
      ...
      { role: 'assistant', text: `You said: "${input}". The execution engine (AGENT-003) will connect this playground to real provider runtimes in the next phase!` }
    ]);
    setInput('');
  };
  ...
  return (
    <ConsoleLayout>
      <div className="h-[calc(100vh-12rem)] flex flex-col justify-between">
        <div>
          <h1 className="text-3xl font-extrabold tracking-tight text-white">Playground</h1>
          <p className="text-zinc-500 text-sm mt-1">Interactively run and debug agent configurations</p>
        </div>
          ...
          <div className="flex-1 overflow-y-auto space-y-4 pr-2">
            {messages.length === 0 ? (
              <div className="h-full flex flex-col items-center justify-center text-center">
                ...
                <h3 className="text-sm font-semibold text-zinc-300">Playground ready</h3>
                <p className="text-xs text-zinc-500 mt-1 max-w-xs">Type a message below to test inputs. Responses will run mocks until the execution engine is built.</p>
              </div>
            ) : (
              messages.map((m, idx) => (
                <div key={idx} className={`flex gap-3 max-w-3xl ${m.role === 'user' ? 'ml-auto flex-row-reverse' : ''}`}>
                  <div className={`h-8 w-8 rounded-lg flex items-center justify-center flex-shrink-0 border ${
                    m.role === 'user' ? 'border-white/[0.08] bg-white/[0.02]' : 'border-indigo-500/20 bg-indigo-500/10'
                  }`}>
                    {m.role === 'user' ? <User className="h-4 w-4 text-zinc-400" /> : <Bot className="h-4 w-4 text-indigo-400" />}
                  </div>
