...
function Navbar({ health }: { health: HealthStatus }) {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${scrolled ? "bg-[#060608]/90 backdrop-blur-xl border-b border-white/[0.05]" : ""}`}
    >
      <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
        ...
        <div className="hidden md:flex items-center gap-3">
          <span
            className={`text-xs px-2.5 py-1 rounded-full border flex items-center gap-1.5 font-medium ${statusColor}`}
          >
            <span
              className={`h-1.5 w-1.5 rounded-full ${statusDot} ${health.status === "healthy" ? "animate-pulse" : ""}`}
            />
            {statusLabel}
          </span>
          <a
            href="https://github.com/sadiab47/AIOps_Hub"
            target="_blank"
            rel="noreferrer"
        ...
        <div className="flex-1 p-4 overflow-hidden">
          ...
          <div className="grid grid-cols-3 gap-2 mb-3">
            {[
              ...
              <div
                key={s.label}
                className="bg-white/[0.03] rounded-lg p-2 border border-white/[0.05]"
              >
            ...
            {agents.map((agent) => (
              ...
              >
                ...
                <div className="flex items-center gap-3">
                  ...
                  <span
                    className={`text-[10px] px-1.5 py-0.5 rounded-full font-medium ${agent.status === "ACTIVE" ? "bg-emerald-500/10 text-emerald-400" : "bg-zinc-500/10 text-zinc-500"}`}
                  >
                    {agent.status}
                  </span>
                </div>
  ...
  return (
    <div className="grid grid-cols-2 md:grid-cols-4 gap-4 my-20" id="stats">
      {stats.map(({ value, label, icon: Icon }) => (
        <div
          key={label}
          className="text-center p-6 rounded-2xl card-glass border border-white/[0.05] hover:border-indigo-500/20 transition-all group"
        >
          <Icon className="h-5 w-5 text-indigo-400 mx-auto mb-3 group-hover:scale-110 transition-transform" />
          <div className="text-3xl font-extrabold gradient-text-primary mb-1">
            {value}
          </div>
  ...
  const caps = [
    {
      ...
      desc: "JWT rotation, RBAC, policy engine, per-org isolation.",
    },
    {
      icon: Bot,
      label: "Agent Registry",
      desc: "Immutable version snapshots, prompt bindings, provider configs.",
    },
    {
      icon: BookOpen,
      label: "Prompt Library",
      desc: "Versioned templates with variable engine and Jinja-style rendering.",
    },
    {
      icon: Cpu,
      label: "5 AI Providers",
      desc: "OpenAI, Claude, Gemini, Azure, Ollama — unified abstraction layer.",
    },
    {
      icon: MessageSquare,
      label: "SSE Streaming Chat",
      desc: "Real-time token streaming with sliding-window and summary memory.",
    },
    {
      icon: BarChart3,
      label: "Usage Analytics",
      desc: "Token cost, latency, provider and model breakdowns, daily trends.",
    },
    {
      icon: Activity,
      label: "Health Probes",
      desc: "Separate /live and /ready probes for Kubernetes-ready deployments.",
    },
    {
      icon: Globe,
      label: "Production Hardened",
      desc: "Rate limiting, helmet, request IDs, structured logging, Zod validation.",
    },
  ];
    ...
    <section className="my-20">
      <div className="text-center mb-12">
        ...
        <p className="text-zinc-500 text-base max-w-xl mx-auto">
          Enterprise-grade architecture spanning auth, AI orchestration,
          telemetry, and operational readiness.
        </p>
      </div>
  ...
  return (
    <section className="my-20">
      <div className="text-center mb-10">
        ...
        <p className="text-zinc-500 text-base max-w-md mx-auto">
          Clean separation between every domain layer.
        </p>
      </div>
      ...
      <div className="flex flex-col items-center gap-0">
        {layers.map((layer, i) => (
          <div key={layer.label} className="flex flex-col items-center">
            <div
              className={`flex items-center gap-3 px-6 py-3 rounded-xl border text-sm font-medium ${layer.color} w-72 justify-center`}
            >
  ...
  return (
    <section className="my-20" id="roadmap">
      <div className="text-center mb-12">
        ...
        <p className="text-zinc-500 text-base max-w-md mx-auto">
          A systematic progression of independently releasable milestones.
        </p>
      </div>
          ...
          {phases.map((phase) => (
            <div key={phase.version} className="relative flex gap-6 group">
              ...
              <div className="hidden md:flex flex-col items-center flex-shrink-0 w-16">
                <div
                  className={`h-4 w-4 rounded-full border-2 z-10 flex-shrink-0 mt-4 ${
                    phase.status === "done"
              ...
              >
                ...
                <div className="flex flex-wrap gap-2">
                  {phase.items.map((item) => (
                    <span
                      key={item}
                      className="text-xs text-zinc-400 bg-white/[0.03] border border-white/[0.05] px-2.5 py-1 rounded-lg"
                    >
                      {item}
                    </span>
                  ))}
    ...
    <section className="my-20">
      <div className="max-w-lg mx-auto p-6 rounded-2xl card-glass border border-white/[0.07] text-center">
        <div className="flex items-center justify-center gap-2 mb-4">
          <span
            className={`h-2 w-2 rounded-full ${health.status === "healthy" ? "bg-emerald-400 animate-pulse" : health.status === "loading" ? "bg-zinc-500 animate-pulse" : "bg-red-400"}`}
          />
        ...
        <div className="flex justify-center gap-4">
          {checks.map(({ label, ok }) => (
            <div key={label} className="flex flex-col items-center gap-1.5">
              <div
                className={`h-8 w-8 rounded-full border flex items-center justify-center ${ok ? "border-emerald-500/30 bg-emerald-500/10" : "border-zinc-700 bg-zinc-800"}`}
              >
                {ok ? (
      ...
      <div className="max-w-7xl mx-auto px-6 py-5 border-t border-white/[0.04] flex flex-col md:flex-row items-center justify-between gap-3">
        <div className="flex items-center gap-3">
          ...
          <span className="text-sm text-zinc-500">
            © {new Date().getFullYear()} AIOps Hub
          </span>
        </div>
      ...
      <main className="max-w-6xl mx-auto px-6 pt-32">
        ...
        <section className="text-center mb-16 animate-fade-up">
          ...
          <h1 className="text-5xl md:text-7xl font-extrabold tracking-tight mb-6 leading-[1.05]">
            ...
            <br />
            <span className="text-zinc-500 text-4xl md:text-5xl font-bold">
              built for production.
            </span>
          </h1>

          <p className="text-zinc-400 text-lg md:text-xl max-w-2xl mx-auto mb-10 leading-relaxed">
            Multi-tenant Agent orchestration, versioned Prompt Library,
            real-time streaming chat, and full Usage Analytics — all in a single
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-4 animate-fade-up-delay-1">
            {[
              ...
              <span
                key={f}
                className="text-xs text-zinc-400 bg-white/[0.03] border border-white/[0.06] px-3 py-1 rounded-full"
              >
                {f}
              </span>
            ))}
        ...
        <p className="text-center text-xs text-zinc-600 mb-6">
          Agent Registry — built in AGENT-001
        </p>
