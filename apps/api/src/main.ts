...
async function bootstrap() {
  ...
  const swaggerConfig = new DocumentBuilder()
    ...
    .addTag(
      "Organizations",
      "Multi-tenant organization management, switching, settings, and profile",
    )
    .addTag(
      "Members",
      "Organization member lifecycle, roles, permissions, ownership transfer, and leave",
    )
    .addTag(
      "Invitations",
      "Organization invitation creation, accept token verification, listing, and revoking",
    )
    .build();
  ...
  console.log(
    `🚀 API Application is running on: http://localhost:${port}/api/v1`,
  );
  console.log(
