...
describe("Pure Resource Authorization Policies (RBAC-002)", () => {
  ...
  describe("MemberPolicy", () => {
    ...
    it("denies access with NOT_FOUND if member does not exist or belongs to another tenant", () => {
      ...
      const outOfTenantResult = MemberPolicy.canManageMember(
        mockOwnerCtx,
        outOfTenantMember,
      );
    ...
    it("denies assigning OWNER role via role change policy", () => {
      const result = MemberPolicy.canManageMember(
        mockOwnerCtx,
        memberTarget,
        OrgRole.OWNER,
      );
    ...
    it("validates ownership transfer policy correctly", () => {
      const nonOwnerResult = MemberPolicy.canTransferOwnership(
        mockAdminCtx,
        memberTarget,
      );
      ...
      const selfResult = MemberPolicy.canTransferOwnership(mockOwnerCtx, {
        ...memberTarget,
        userId: "owner-user-id",
      });
      ...
      const validResult = MemberPolicy.canTransferOwnership(
        mockOwnerCtx,
        memberTarget,
      );
  ...
  describe("InvitationPolicy", () => {
    ...
    it("allows managing invitation in same tenant context", () => {
      const result = InvitationPolicy.canManageInvitation(
        mockAdminCtx,
        inviteResource,
      );
      expect(result.allowed).toBe(true);
  ...
  describe("OrganizationPolicy", () => {
    it("allows updating settings for active tenant context", () => {
      const result = OrganizationPolicy.canUpdateSettings(
        mockOwnerCtx,
        "org-1",
      );
      expect(result.allowed).toBe(true);
