...
export class ChatController {
  ...
  async create(@TenantId() orgId: string, @Body() dto: CreateConversationDto) {
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "Open real-time token stream (SSE) for prompt message",
  })
  @ApiParam({ name: "id", type: String, format: "uuid" })
  @ApiResponse({
    status: 200,
    description: "Token stream connection established.",
  })
  async stream(
    @TenantId() orgId: string,
    @Param("id", ParseUUIDPipe) id: string,
