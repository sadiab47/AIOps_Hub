...
export class TextUtilsTool implements AgentTool {
  readonly definition: ToolDefinition = {
    name: 'text_utilities',
    description: 'Perform text manipulations: lowercase, uppercase, trim, count words/chars, or replace substrings.',
    inputSchema: {
      type: 'object',
      properties: {
        action: {
        ...
        replaceTarget: {
          type: 'string',
          description: 'The substring to search for (only required for replace action).',
        },
        replaceValue: {
          type: 'string',
          description: 'The string replacing the target (only required for replace action).',
        },
      },
    ...
    switch (action) {
      ...
      case 'replace':
        if (replaceTarget === undefined || replaceValue === undefined) {
          throw new Error("Parameters 'replaceTarget' and 'replaceValue' are required for action 'replace'.");
        }
