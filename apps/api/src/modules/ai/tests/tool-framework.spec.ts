...
describe('Tool Framework (AGENT-002)', () => {
  ...
  describe('ToolExecutor', () => {
    ...
    it('should return failure result on invalid schema inputs', async () => {
      const result = await executor.execute(mockTool, { value: 'not-a-number' });
  ...
  describe('Built-in Tools', () => {
    describe('CalculatorTool', () => {
      ...
      it('should add, subtract, multiply, and divide numbers', async () => {
        await expect(calculator.execute({ operation: 'add', a: 10, b: 5 })).resolves.toBe(15);
        await expect(calculator.execute({ operation: 'subtract', a: 10, b: 5 })).resolves.toBe(5);
        await expect(calculator.execute({ operation: 'multiply', a: 10, b: 5 })).resolves.toBe(50);
        await expect(calculator.execute({ operation: 'divide', a: 10, b: 5 })).resolves.toBe(2);
      });

      it('should throw error on division by zero', async () => {
        await expect(calculator.execute({ operation: 'divide', a: 10, b: 0 })).rejects.toThrow(
    ...
    describe('DateTimeTool', () => {
      ...
      it('should reject invalid timezones', async () => {
        await expect(datetime.execute({ timezone: 'Invalid/Zone' })).rejects.toThrow();
      });
    });
    ...
    describe('JsonUtilsTool', () => {
      ...
      it('should format, minify, and parse json payloads', async () => {
        ...
        const stringified = await jsonUtils.execute({ action: 'stringify', payload, spaces: 4 });
    ...
    describe('TextUtilsTool', () => {
      ...
      it('should lowercase, uppercase, trim, stats, and replace texts', async () => {
        await expect(textUtils.execute({ action: 'lowercase', text: 'HELLO' })).resolves.toBe('hello');
        await expect(textUtils.execute({ action: 'uppercase', text: 'world' })).resolves.toBe('WORLD');
        await expect(textUtils.execute({ action: 'trim', text: '  text  ' })).resolves.toBe('text');

        const stats = await textUtils.execute({ action: 'stats', text: 'Hello world\nAnother line' });
    ...
    describe('HttpClientTool', () => {
      ...
      it('should reject access to domains not in the allow-list', async () => {
        await expect(
          client.execute({ method: 'GET', url: 'https://malicious-endpoint.com/api/data' }),
        ).rejects.toThrow('Domain \'malicious-endpoint.com\' is not in the allow-list');
      });

      it('should request allow-listed domains successfully (using mock endpoint)', async () => {
        // Mock fetch globally
        const mockResponse = { status: 200, headers: new Map([['content-type', 'application/json']]), json: async () => ({ ok: true }) };
        const originalFetch = global.fetch;
        global.fetch = jest.fn().mockResolvedValue(mockResponse);

        const result = await client.execute({ method: 'GET', url: 'https://httpbin.org/json' });
