...
describe("Agent Execution Engine (AGENT-003)", () => {
  ...
  beforeAll(async () => {
    ...
    } = require("../../../common/interceptors/response-envelope.interceptor");
    const {
    ...
    const {
      AiProviderFactory,
    } = require("../../../common/ai/factories/ai-provider.factory");
    ...
    const mockOpenAiProvider = {
      ...
      generateCompletion: jest.fn().mockImplementation((req) => {
        const findMessage = (flag: string) =>
          req.messages.some((m: any) => m.content && m.content.includes(flag));
        ...
        if (findMessage("TRIGGER_TOOL_NOT_FOUND")) {
          ...
          if (
            lastMsg.role === "tool" &&
            lastMsg.content.includes("not found")
          ) {
            ...
            toolCalls: [
              {
                id: "call-2",
                type: "function",
                function: { name: "tool_not_found", arguments: "{}" },
              },
            ],
            usage: { promptTokens: 5, completionTokens: 5 },
          };
    ...
    const accessCookie = rawCookies.find((c: string) =>
      c.startsWith("aiops_access_token="),
    );
    ...
    const {
      CredentialService,
    } = require("../../../common/ai/services/credential.service");
    const credentialService = moduleFixture.get(CredentialService);
    const encryptedCredentials = credentialService.encryptCredentials({
      apiKey: "MOCK_API_KEY",
    });
  ...
  it("should successfully trigger simple prompt executions and verify telemetry & events", async () => {
    ...
    const execution = await prisma.agentExecution.findFirst({
      where: {
        agentId,
        organizationId: orgId,
        input: { equals: "Explain quantum physics in one sentence." },
      },
    });
  ...
  it("should handle unauthorized tool execution failure", async () => {
    ...
    const execution = await prisma.agentExecution.findFirst({
      where: {
        agentId,
        organizationId: orgId,
        input: { equals: "TRIGGER_UNAUTHORIZED_TOOL" },
      },
    });
    expect(execution?.status).toBe("FAILED");
  ...
  it("should handle missing tool resolver failure gracefully", async () => {
    ...
    const execution = await prisma.agentExecution.findFirst({
      where: {
        agentId,
        organizationId: orgId,
        input: { equals: "TRIGGER_TOOL_NOT_FOUND" },
      },
    });
    expect(execution?.status).toBe("FAILED");
    expect(execution?.errorMessage).toContain(
      "Tool 'tool_not_found' not found",
    );

    // Clean up
    await prisma.agentTool.deleteMany({
      where: { agentId, toolId: "tool_not_found" },
    });
  });
  ...
  it("should successfully execute valid tools and track event logs", async () => {
    ...
    await prisma.agentTool.deleteMany({
      where: { agentId, toolId: "calculator" },
    });
  ...
  it("should stop and fail if max iterations loop limit is exceeded", async () => {
    ...
    const execution = await prisma.agentExecution.findFirst({
      where: {
        agentId,
        organizationId: orgId,
        input: { equals: "TRIGGER_TOOL_LOOP" },
      },
    });
    expect(execution?.status).toBe("FAILED");
    expect(execution?.errorMessage).toContain(
      "exceeded maximum iterations cap",
    );
