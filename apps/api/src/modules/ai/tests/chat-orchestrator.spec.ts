...
describe("ChatOrchestrator", () => {
  ...
  beforeEach(async () => {
    repository = {
      ...
      createMessage: jest
        .fn()
        .mockResolvedValue({ id: "msg-1", role: MessageRole.USER }),
    ...
    conversationMemoryService = {
      buildContext: jest
        .fn()
        .mockResolvedValue([{ role: "user", content: "hi {{name}}" }]),
    };
    ...
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        { provide: CONVERSATION_REPOSITORY_TOKEN, useValue: repository },
        {
          ...
          useValue: conversationMemoryService,
        },
  ...
  describe("streamMessage", () => {
    ...
    it("should orchestrate template rendering and stream token chunks successfully", async () => {
      ...
      expect(events).toContainEqual({
        event: "metadata",
        data: { provider: AiProviderEnum.OPENAI, model: "gpt-4o" },
      });
      expect(events).toContainEqual({
        event: "token",
        data: { token: "Hello" },
