...
describe("Prompt Library Integration Tests (AI-002)", () => {
  ...
  beforeAll(async () => {
    ...
    const cat = await prisma.promptCategory.findFirst({
      where: { name: "Support" },
    });
    categoryId = cat!.id;
  });
  ...
  describe("REST Endpoints CRUD & Preview rendering", () => {
    ...
    it("should list seeded prompt categories", async () => {
      ...
      expect(
        res.body.data.some((c: { name: string }) => c.name === "Support"),
      ).toBe(true);
    });
    ...
    it("should add a new version v2 to version history", async () => {
      const res = await request(app.getHttpServer())
        ...
        .send({
          template:
            "Dear {{customerName}}, welcome to the revamped {{company}}! Support ID: {{id}}",
    ...
    it("should render template version preview reporting diagnostics", async () => {
      ...
      expect(res.body.data.rendered).toContain(
        "Dear John, welcome to the revamped Google! Support ID: ",
      );
      expect(res.body.data.variables).toContain("customerName");
