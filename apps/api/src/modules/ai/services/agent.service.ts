  ...
  async create(
    ...
    if (nameExists) {
      throw new ConflictException(
        `Agent with name "${dto.name}" already exists in this organization`,
      );
    }
    ...
    if (slugExists) {
      throw new ConflictException(
        `Agent with slug "${dto.slug}" already exists in this organization`,
      );
    }
    ...
    if (!providerConfig) {
      throw new BadRequestException(
        "Selected AI Provider configuration does not exist or belongs to another tenant",
      );
    }
    ...
    if (dto.version.promptVersionId) {
      ...
      if (!promptVersion || promptVersion.prompt.organizationId !== orgId) {
        throw new BadRequestException(
          "Selected Prompt version does not exist or belongs to another tenant",
        );
      }
    }
