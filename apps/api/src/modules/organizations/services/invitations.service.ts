...
export class InvitationsService {
  ...
  async invite(
    ...
    const pendingInvite =
      await this.invitationRepository.findPendingByEmailAndOrg(email, orgId);
    if (pendingInvite) {
      // Reuse and extend expiry
      const updatedInvite = await this.invitationRepository.update(
        pendingInvite.id,
        {
          tokenHash,
          expiresAt,
  ...
  async getInvitationMetadata(token: string) {
    ...
    const invite =
      await this.invitationRepository.findActiveByTokenHash(tokenHash);
  ...
  async accept(
    ...
    const existingMember = await this.memberRepository.findMembership(
      userId,
      invite.organizationId,
    );
    if (existingMember) {
  ...
  async revoke(
    ...
    const policyResult = this.authorizationService.canManageInvitation(
      actorCtx,
      invite,
    );
    if (!policyResult.allowed || !invite) {
      throw new NotFoundException(
        policyResult.reason ?? "Invitation not found",
      );
    }
