...
describe("OrganizationsService", () => {
  ...
  describe("create", () => {
    it("should generate a unique slug and call repository transaction", async () => {
      ...
      organizationRepository.createWithMemberAndAudit.mockResolvedValue(
        mockOrg,
      );
      ...
      expect(organizationRepository.existsBySlug).toHaveBeenCalledWith(
        "acme-corp",
      );
      expect(
        organizationRepository.createWithMemberAndAudit,
    ...
    it("should sequentially suffix the slug if it already exists", async () => {
      ...
      expect(organizationRepository.existsBySlug).toHaveBeenNthCalledWith(
        1,
        "acme",
      );
      expect(organizationRepository.existsBySlug).toHaveBeenNthCalledWith(
      ...
      expect(
        organizationRepository.createWithMemberAndAudit,
      ).toHaveBeenCalledWith({ name: "Acme", slug: "acme-3" }, "user-uuid");
      expect(result).toEqual(mockOrg);
    });
  });
  ...
  describe("listUserOrganizations", () => {
    it("should retrieve organizations list with roles from repository", async () => {
      ...
      expect(organizationRepository.findUserOrganizations).toHaveBeenCalledWith(
        "user-uuid",
      );
  ...
  describe("switchOrganization", () => {
    it("should throw ForbiddenException if user has no membership in organization", async () => {
      ...
      await expect(
        service.switchOrganization("user-uuid", "org-uuid"),
    ...
    it("should return context schema if membership is valid", async () => {
      ...
      expect(
        organizationRepository.findOrganizationContext,
      ).toHaveBeenCalledWith("user-uuid", "org-uuid");
  ...
  describe("updateProfileAndSettings", () => {
    ...
    it("should throw ForbiddenException if user lacks administrative access", async () => {
      ...
      await expect(
        service.updateProfileAndSettings("user-uuid", "org-uuid", {
    ...
    it("should throw BadRequestException if logoUrl is not HTTPS", async () => {
      await expect(
        service.updateProfileAndSettings("user-uuid", "org-uuid", {
          settings: { logoUrl: "http://logo.png" },
        }),
      ).rejects.toThrow(BadRequestException);
    });
    ...
    it("should invoke transaction and publish domain events on valid updates", async () => {
      ...
      organizationRepository.updateProfileAndSettings.mockResolvedValue({
        success: true,
      } as any);
      ...
      await service.updateProfileAndSettings(
        "user-uuid",
        "org-uuid",
        payload,
        "127.0.0.1",
      ...
      expect(
        organizationRepository.updateProfileAndSettings,
      ).toHaveBeenCalledWith(
        "org-uuid",
        { name: "Acme Corp", slug: "acme-corp" },
