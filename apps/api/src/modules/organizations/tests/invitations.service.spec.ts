...
describe("InvitationsService", () => {
  ...
  describe("invite", () => {
    ...
    it("should reuse and extend duplicate pending invitations", async () => {
      ...
      invitationRepository.findPendingByEmailAndOrg.mockResolvedValue(
        mockPending,
      );
      ...
      expect(
        invitationRepository.findPendingByEmailAndOrg,
      ).toHaveBeenCalledWith("test@example.com", "org-uuid");
  ...
  describe("getInvitationMetadata", () => {
    it("should throw NotFoundException if invitation does not exist/expired", async () => {
      ...
      await expect(service.getInvitationMetadata("dummy")).rejects.toThrow(
        NotFoundException,
      );
    });
