...
describe("Members Integration Tests (ORG-005)", () => {
  ...
  beforeAll(async () => {
    ...
    const rawToken = await sendInvite(
      ownerCookies,
      memberEmail,
      OrgRole.MEMBER,
    );
  ...
  describe("GET /organizations/members/:memberId", () => {
    ...
    it("returns 404 for non-existent member ID", async () => {
      await request(app.getHttpServer())
        .get(
          "/api/v1/organizations/members/00000000-0000-0000-0000-000000000000",
        )
        .set("Cookie", ownerCookies)
        .set("x-organization-id", orgId)
        .expect(404);
  ...
  describe("DELETE /organizations/members/:memberId", () => {
    ...
    it("allows OWNER to remove a MEMBER and verifies audit log", async () => {
      ...
      const throwawayUser = await prisma.user.findUnique({
      ...
      const throwawayCookies = throwawayLogin.headers[
        "set-cookie"
  ...
  describe("POST /organizations/members/:memberId/transfer-owner", () => {
    ...
    it("returns 404 when target member does not exist", async () => {
      await request(app.getHttpServer())
        .post(
          "/api/v1/organizations/members/00000000-0000-0000-0000-000000000000/transfer-owner",
        )
        .set("Cookie", ownerCookies)
        .set("x-organization-id", orgId)
        .expect(404);
  ...
  describe("POST /organizations/leave", () => {
    ...
    it("allows a MEMBER to leave the organization", async () => {
      ...
      // Verify soft-delete
      const leftMember = await prisma.member.findUnique({
        where: { id: leaverMember!.id },
      });
