...
const ownerMembership = makeMember({
...
const memberMembership = makeMember({
  id: "member-record-uuid",
  userId: MEMBER_USER_ID,
  role: OrgRole.MEMBER,
});
...
describe("MemberManagementService", () => {
  ...
  describe("listMembers", () => {
    it("returns mapped MemberSummaryDto array", async () => {
      mockMemberRepository.findMembersByOrganization.mockResolvedValue([
        memberMembership,
      ]);
  ...
  describe("getMember", () => {
    ...
    it("throws NotFoundException when member not in org", async () => {
      ...
      await expect(service.getMember(ORG_ID, "unknown-id")).rejects.toThrow(
        NotFoundException,
      );
    });
  });
  ...
  describe("changeRole", () => {
    it("updates role successfully when OWNER acts on MEMBER", async () => {
      ...
      mockMemberRepository.findMemberById.mockResolvedValue(memberMembership);
      mockMemberRepository.updateRole.mockResolvedValue({
        ...memberMembership,
        role: OrgRole.ADMIN,
      });
      ...
      expect(mockMemberRepository.updateRole).toHaveBeenCalledWith(
        memberMembership.id,
        OrgRole.ADMIN,
      );
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        expect.any(MemberRoleChangedEvent),
      );
    });
    ...
    it("allows ADMIN to change role of MANAGER", async () => {
      const managerTarget = makeMember({
        id: "mgr-uuid",
        userId: "mgr",
  ...
  describe("transferOwnership", () => {
    it("transfers ownership atomically", async () => {
      ...
      expect(mockEventBus.publish).toHaveBeenCalledWith(
        expect.any(OwnershipTransferredEvent),
      );
    });
