...
export class InvitationsController {
  ...
  @ApiResponse({
    status: 403,
    description: "Forbidden — requires invitation.create permission.",
  })
  @ApiResponse({
    status: 409,
    description:
      "User is already an active member or pending invitation exists.",
  })
  async invite(
  ...
  @ApiOperation({
    summary: "List all pending invitations for the active organization",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 403,
    description: "Forbidden — requires invitation.view permission.",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Invitation metadata retrieved successfully.",
  })
  ...
  @ApiResponse({
    status: 200,
    description: "Invitation accepted and member created.",
  })
  @ApiResponse({ status: 401, description: "Unauthenticated." })
  @ApiResponse({
    status: 403,
    description: "Invitation email does not match authenticated user email.",
  })
  @ApiResponse({ status: 404, description: "Invitation not found or expired." })
  async accept(
  ...
  @ApiResponse({
    status: 403,
    description: "Forbidden — requires invitation.revoke permission.",
  })
  @ApiResponse({
    status: 404,
    description: "Invitation not found in active organization.",
  })
  async revoke(
    @Param("id") id: string,
    @TenantId() orgId: string,
