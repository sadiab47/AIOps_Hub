...
export class OrganizationsController {
  ...
  @ApiResponse({
    status: 400,
    description: "Validation or duplicate organization name.",
  })
  ...
  @ApiResponse({ status: 401, description: "Unauthenticated." })
  async listUserOrganizations(@CurrentUser("sub") userId: string) {
  ...
  @ApiOperation({
    summary: "Update organization profile and configuration settings",
  })
  @ApiResponse({
    status: 200,
  ...
  @ApiResponse({
    status: 403,
    description:
      "Forbidden administrative settings access (Requires settings.update permission).",
  })
