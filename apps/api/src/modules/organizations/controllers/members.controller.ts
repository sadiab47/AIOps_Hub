...
export class MembersController {
  ...
  @ApiOperation({
    summary: "List all active members of the current organization",
  })
  ...
  @ApiResponse({
    status: 403,
    description:
      "Not a member of this organization or missing member:list permission.",
  })
  ...
  @ApiResponse({
    status: 409,
    description:
      "Last owner must transfer ownership or delete the organization before leaving.",
  })
  async leave(
  ...
  @ApiOperation({
    summary: "Change a member's role (OWNER cannot be assigned here)",
  })
  @ApiParam({ name: "memberId", type: String, format: "uuid" })
  @ApiResponse({ status: 200, description: "Role updated successfully." })
  @ApiResponse({
    status: 400,
    description: "Invalid role (e.g. OWNER attempted via this endpoint).",
  })
  @ApiResponse({ status: 401, description: "Unauthenticated." })
  @ApiResponse({
    status: 403,
    description: "Forbidden — insufficient permission or role matrix conflict.",
  })
  @ApiResponse({ status: 404, description: "Member not found." })
  ...
  async remove(
    ...
    await this.memberManagementService.removeMember(actorId, orgId, memberId, {
      userId: actorId,
      organizationId: orgId,
      ipAddress: ip || null,
      userAgent: userAgent || null,
  ...
  @HttpCode(HttpStatus.OK)
  @ApiOperation({
    summary: "Atomically transfer organization ownership to another member",
  })
  @ApiParam({ name: "memberId", type: String, format: "uuid" })
  @ApiResponse({
    status: 200,
